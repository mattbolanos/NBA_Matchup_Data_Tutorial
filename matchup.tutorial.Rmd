---
title: "NBA Matchup Data Scraping Tutorial"
author: "Matt Bolaños"
...
Before we start gathering the matchup data, we need to load the necessary packages.
...
library(data.table)
```
You don't need to understand why the below code works (I don't have a deep understanding of them myself), but it essentially sets the headers for us to request data from the NBA API.
```{r,results=FALSE}
headers <- c(
...
```
Then, we pick the games and their IDs from nbastatR. To pick the games you want, browse the seasons_schedule() function and filter for those games. Below are the 6 games from the 2021 Western Conference Finals, and the ids variable contains the IDs we need. 
...
ids <- games$idGame
```
This function parses the JSON response from the NBA's site and returns the matchup data from the game IDs provided in a dataframe.
```{r}
...
```
And to use the function, we will use a dataframe mapping function from the furrr package.
```{r}
